...
class BaseChart(p.SingletonPlugin):
    ...
    def info(self):
        schema = {
            'width': [ignore_missing],
            'height': [ignore_missing],
            'y_axis': [not_empty],
            'color': [not_empty],
    ...
    def setup_template_variables(self, context, data_dict):
        ...
        colors = [
            {'value': 'interpolateGreys', 'text': 'Peleka (Greys)'},
            {'value': 'interpolateGreens', 'text': 'Zala (Greens)'},
            {'value': 'interpolateBlues', 'text': 'Zila (Blues)'},
            {'value': 'interpolateBuPu', 'text': 'Zila - violeta (BuPu)'},
            {'value': 'interpolateRainbow', 'text': 'Varaviksne (Rainbow)'},
            {'value': 'interpolateGnBu', 'text': 'Zala - zila (GrBu)'}
        ]
...
class ClusterChart(BaseChart):
    ...
    def info(self):
        info = super(ClusterChart, self).info()
        info['schema'] = {
            'id_field': [not_empty],
            'parent_id_field': [not_empty],
            'count_field': [not_empty],
            'name_field': [not_empty],
            'width': [ignore_missing],
            'height': [ignore_missing],
            'color': [not_empty]
        }
        info['name'] = 'cluster'
        info['title'] = u'Hierarhijas grafiks'
        info['filterable'] = False

        return info

    def form_template(self, context, data_dict):
        return 'clusterchart_form.html'

class CorelationChart(BaseChart):

    CHART_TYPE = 'corelation'

    def info(self):
        info = super(CorelationChart, self).info()
        info['schema'] = {
            'width': [ignore_missing],
            'height': [ignore_missing],
            'color': [not_empty]
        }
        info['name'] = 'corelation'
        info['title'] = u'Korelaciju grafiks'
        info['filterable'] = False

        return info

    def form_template(self, context, data_dict):
        return 'corelationchart_form.html'

class SunburstChart(ClusterChart):

    CHART_TYPE = 'sunburst'

    def info(self):
        info = super(SunburstChart, self).info()
        info['name'] = 'sunburst'
        info['title'] = u'Hierarhijas svaru grafiks'
...
class HexbinChart(LineChart):

    CHART_TYPE = 'hexbin'

    def info(self):
        info = super(HexbinChart, self).info()
        info['name'] = 'hexbin'
        info['title'] = u'Dispersijas grafiks'
...
class BoxplotChart(LineChart):
    ...
    def info(self):
        info = super(BoxplotChart, self).info()
        info['name'] = 'boxplot'
        info['title'] = u'Distribucijas grafiks'
